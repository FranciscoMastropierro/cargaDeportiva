...
function physicalStatusLabel(status: string) {
  return status === "injured" ? "Lesionado" : status === "minor_issue" ? "Molestia" : "Disponible";
}
...
export default async function PlayersPage({ searchParams }: { searchParams: Promise<{ edit?: string }> }) {
  ...
  return <section className="space-y-5">
    ...
    <ConfirmationForm key={selected?.id ?? "new-player"} action={savePlayer} kind="player" confirm={Boolean(selected)} className="card grid gap-3 md:grid-cols-4">
      ...
      <label className="grid gap-1 text-sm font-medium">Nombre<input name="name" defaultValue={selected?.name ?? ""} required /></label>
      <label className="grid gap-1 text-sm font-medium">Posición<select name="position" defaultValue={selected?.position ?? ""} required><option value="" disabled>Elegir</option>{FOOTBALL_POSITIONS.map((position) => <option key={position} value={position}>{position}</option>)}</select></label>
